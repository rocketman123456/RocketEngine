...
namespace Rocket {
    namespace FileSystem {
        ...
        {
        public:
            ZipFile() { entries_num_=0; file_=NULL; raw_data_=NULL; }
            virtual ~ZipFile() { End(); fclose(file_); }

            bool Init(const std::string &resFileName);
            void End();

            int GetNumFiles()const { return entries_num_; }
        ...
        private:
            ...
            FILE* file_;		// Zip file
            char* raw_data_;	// Raw data buffer.
            int  entries_num_;	// Number of entries.
