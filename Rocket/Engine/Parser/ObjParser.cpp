...
namespace Rocket {
    ...
    bool ObjParser::Parse() {
        ...
        SplitMultiChar(content_, &result, "\n");
        ...
        for(auto curline : result) {
            ...
            if (FirstToken(curline) == "v") {
                ...
                Eigen::Vector3f vpos;
                SplitMultiChar(Tail(curline), &spos, " ");
            ...
            if (FirstToken(curline) == "usemtl") {
                mesh_mat_names.push_back(Tail(curline));
            ...
            if (FirstToken(curline) == "mtllib") {
                ...
                SplitMultiChar(full_path_, &temp, "/");
    ...
    void ObjParser::GenVerticesFromRawOBJ(
        ...
        for (int i = 0; i < int(sface.size()); i++) {
            ...
            switch (vtype) {
                ...
                case 2: { // P/T
                    vVert.position = GetElement(iPositions, svert[0]);
                    vVert.texture_coordinate = GetElement(iTCoords, svert[1]);
                    noNormal = true;
                    oVerts.push_back(vVert);
                    break;
                }
    ...
    bool ObjParser::LoadMaterials(std::string path) {
        ...
        for(auto curline : result) {
            ...
            if (FirstToken(curline) == "Ka") {
                std::vector<std::string> temp;
                SplitMultiChar(Tail(curline), &temp, " ");
                if (temp.size() != 3)
                    continue;
            ...
            if (FirstToken(curline) == "map_Bump" || 
                FirstToken(curline) == "map_bump" || 
                FirstToken(curline) == "bump") {
