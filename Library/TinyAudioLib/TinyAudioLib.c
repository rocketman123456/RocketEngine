...
PaError initPablio(void)
{
    ...
    else
    {
        ...
        if (in_dev_ndx >= 0 && in_latency < 0)
        { /* suggested latency unselected, using default */
    ...
    else
    {
        ...
        if (out_dev_ndx >= 0 && out_latency < 0)
        { /* suggested latency unselected, using default */
...
void tinyExit()
{
    if (isOpened)
    {
        ...
        printf("tinyAudioLib wasn't initialized. Doing nothing\n");
}
    ...
    */
    ...
    { // -2 == choose port ID
        ...
        for (j = 0; j < Pa_GetDeviceCount(); j++)
        {
            ...
            if (info->maxOutputChannels >= 2 && Pa_GetHostApiInfo(info->hostApi)->type == paMME)
            { // only stereo MME ports allowed
                ...
                info->maxOutputChannels); printf("Default low input latency %lf, high input latency %lf\n",
                info->defaultLowInputLatency, info->defaultHighInputLatency); printf("Default low output latency %lf,
                high output latency %lf\n", info->defaultLowOutputLatency, info->defaultHighOutputLatency); */
            }
        }
        printf("==========================\n");
...
PaDeviceID SelectInPort(int port)
{
    ...
    */
    ...
    { // -2 == choose port ID
        ...
        {
            info = Pa_GetDeviceInfo(j);
            if (info->maxInputChannels == 2 && Pa_GetHostApiInfo(info->hostApi)->type == paMME)
            { // only stereo ports allowed
                printf("%d) Name: [%s] %s\n", j, Pa_GetHostApiInfo(info->hostApi)->name, info->name);
                /* printf("Default samplig rate: %lf\n", info->defaultSampleRate);
                printf("Max Input channels %d, Max Output channels %d\n", info->maxInputChannels,
                info->maxOutputChannels); printf("Default low input latency %lf, high input latency %lf\n",
...
void setOutPort(int port) { outPortID = SelectOutPort(port); }

void setInPort(int port) { inPortID = SelectInPort(port); }
