...
int64_t* stack;// stack
int64_t* old_text; // for dump text segment
char*    data; // data segment
int64_t* idmain;

char* src;      // pointer to source code string;
char* old_src;  // pointer to source code string;

int64_t poolsize;   // default size of text/data/stack
int64_t* pc;        // virtual machine registers
int64_t* bp;        // virtual machine registers
int64_t* sp;        // virtual machine registers
int64_t ax;         // virtual machine registers
int64_t cycle;      // virtual machine registers

int64_t* current_id; // current parsed ID
int64_t* symbols;    // symbol table
int64_t line;        // line number of source code
int64_t token_val;   // value of current token (mainly for number)

int64_t basetype;    // the type of a declaration, make it global for convenience
int64_t expr_type;   // the type of an expression
...
// 5: local var 1
// 6: local var 2
int64_t index_of_bp; // index of bp pointer on stack
...
void next() {
    ...
    while (token = *src) {
        ...
        if (token == '\n') {
            if (assembly) {
                // print compile info
                printf("%lld: %.*s", line, (char*)src-old_src, old_src);
                old_src = src;
                while (old_text < text) {
                    printf("%8.4s", &asm_name[*++old_text * 5]);
                    if (*old_text <= ADJ)
                        printf(" %lld\n", *++old_text);
                    else
                        printf("\n");
    ...
    int64_t tmp;
    {
        ...
        if (token == Num) {
            ...
            *++text = token_val;
            ...
            // now only `sizeof(int64_t)`, `sizeof(char)` and `sizeof(*...)` are
            // supported.
            match(Sizeof);
            match('(');
            expr_type = INT;
            ...
            // emit code
            *++text = IMM;
            *++text = (expr_type == CHAR) ? sizeof(char) : sizeof(int64_t);
            expr_type = INT;
        }
        else if (token == Id) {
            ...
            *++text = PUSH;
            *++text = IMM;
            *++text = (expr_type > PTR) ? sizeof(int64_t) : sizeof(char);
...
void statement() {
    ...
    int64_t *a, *b; // bess for branch control
...
void function_body() {
    ...
    int64_t pos_local; // position of local variables on the stack.
    int64_t type;
...
void global_declaration() {
    ...
    int64_t type; // tmp, actual type for variable
    int64_t i; // tmp
    ...
    while (token != ';' && token != '}') {
        type = basetype;
        // parse pointer type, note that there may exist `int64_t ****x;`
        while (token == Mul) {
            match(Mul);
            type = type + PTR;
        }
        ...
        if (token == '(') {
            ...
            current_id[Value] = (int64_t)(text + 1); // the memory address of function
...
int64_t eval() {
    ...
    while (1) {
        ...
        // print debug info
        if (debug) {
            printf("%lld> %.4s", cycle, &asm_name[op * 5]);
            if (op <= ADJ)
                printf(" %lld\n", *pc);
            else
                printf("\n");
        ...
        else if (op == LI)   {ax = *(int64_t *)ax;}                            // load integer to ax, address in ax
        else if (op == SC)   {ax = *(char *)*sp++ = ax;}                       // save character to address, value in ax, address on stack
        else if (op == SI)   {*(int64_t *)*sp++ = ax;}                         // save integer to address, value in ax, address on stack
        else if (op == PUSH) {*--sp = ax;}                                     // push the value of ax onto the stack
        else if (op == JMP)  {pc = (int64_t *)*pc;}                            // jump to the address
        else if (op == JZ)   {pc = ax ? pc + 1 : (int64_t *)*pc;}              // jump if ax is zero
        else if (op == JNZ)  {pc = ax ? (int64_t *)*pc : pc + 1;}              // jump if ax is not zero
        else if (op == CALL) {*--sp = (int64_t)(pc+1); pc = (int64_t *)*pc;}   // call subroutine
        //else if (op == RET)  {pc = (int64_t *)*sp++;}                        // return from subroutine;
        else if (op == ENT)  {*--sp = (int64_t)bp; bp = sp; sp = sp - *pc++;}  // make new stack frame
        else if (op == ADJ)  {sp = sp + *pc++;}                                // add esp, <size>
        else if (op == LEV)  {sp = bp; bp = (int64_t *)*sp++; pc = (int64_t *)*sp++;} // restore call frame and PC
        else if (op == LEA)  {ax = (int64_t)(bp + *pc++);}                     // load address for arguments.
        ...
        else if (op == READ) { ax = read((int)sp[2], (char *)sp[1], (int)*sp); }
...
int main(int argc, char** argv) {
    ...
    src = "char else enum if int64_t return sizeof while "
